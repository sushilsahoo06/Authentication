...
export default function Emailverify() {
  ...
  const { userData,setuserData, isLoggedin, backendURL } = useContext(AppContent);
  const navigate = useNavigate();

  const handleInput = (e, index) => {//when fill the input then it directly went to second input
    if (e.target.value.length > 0 && index < inputRef.current.length - 1) {
      inputRef.current[index + 1].focus();
    }
  };
  const handleKeyDown = (e, index) => {//back
